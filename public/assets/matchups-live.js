import { sleeperManager } from "./rivalry-week.js?v=2";
...
export async function renderMatchupsHub(container, {
  ...
  async function selectView(view, { updateUrl = true } = {}) {
    ...
    if (updateUrl) {
      const hash = view === "live" ? "#live" : view === "schedule" ? "#schedule" : "#archives";
  ...
  window.addEventListener("hashchange", () => {
    const view = window.location.hash === "#schedule" ? "schedule"
      : window.location.hash === "#archives" ? "archives" : "live";
    selectView(view, { updateUrl: false });
  });

  await loadWeek(currentWeek, "live-matchup-grid", { live: true });
  const initialView = window.location.hash === "#schedule" ? "schedule"
    : window.location.hash === "#archives" ? "archives" : "live";
  await selectView(initialView, { updateUrl: false });
}
